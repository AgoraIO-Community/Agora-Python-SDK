...
namespace agora
{
	namespace common
	{
		...
		void
		RtcChannelBridge::remove_C_ChannelEventHandler()
		{
			if (channelEventHandler)
			{
				delete(channelEventHandler);
        ...
        {
            LOG_JSON(REGISTER_MEDIA_META_DATA_OBSERVER, CHANNEL_ID_KEY, channel->channelId(), "type", type, "useSdkDefault", useSdkDefault);
            int ret;
            if (useSdkDefault)
            {
		...
		{
            LOG_JSON(CREATE_DATA_STREAM, CHANNEL_ID_KEY, channel->channelId(), "reliable", reliable, "ordered", ordered);
		...
		{
            LOG_JSON(SEND_STREAM_MESSAGE, CHANNEL_ID_KEY, channel->channelId(), "streamId", streamId, "length", (uint64_t)length);
		...
		{
            LOG_JSON(JOIN_CHANNEL, CHANNEL_ID_KEY, channel->channelId(), "token", token, "info", info, "uid", uid, "options", options);
		...
		RtcChannelBridge::joinChannelWithUserAccount(const char* token,
			...
			const ChannelMediaOptions& options)
		{
            LOG_JSON(JOIN_CHANNEL_WITH_USER_ACCOUNT, CHANNEL_ID_KEY, channel->channelId(), "token", token, "userAccount", userAccount, "options", options);
		...
		{
            LOG_JSON(SET_ENCRYPTION_SECTRT, CHANNEL_ID_KEY, channel->channelId(), "secret", secret);
		...
		{
            LOG_JSON(SET_ENCRYPTION_MODE, CHANNEL_ID_KEY, channel->channelId(), "encryptionMode", encryptionMode);
		...
		int
		RtcChannelBridge::setRemoteUserPriority(rtc::uid_t uid, PRIORITY_TYPE userPriority)
		{
            LOG_JSON(SET_REMOTE_USER_PRIORITY, CHANNEL_ID_KEY, channel->channelId(), "uid", uid, "userPriority", userPriority);
			return channel->setRemoteUserPriority(uid, userPriority);
		}

		int
		RtcChannelBridge::setRemoteVoicePosition(rtc::uid_t uid, double pan, double gain)
		{
            LOG_JSON(SET_REMOTE_VOICE_POSITIONN, CHANNEL_ID_KEY, channel->channelId(), "uid", uid, "pan", pan, "gain", gain);
			return channel->setRemoteVoicePosition(uid, pan, gain);
		}

		int
		RtcChannelBridge::setRemoteRenderMode(rtc::uid_t userId,
			RENDER_MODE_TYPE renderMode,
			VIDEO_MIRROR_MODE_TYPE mirrorMode)
		{
            LOG_JSON(SET_REMOTE_RENDER_MODE, CHANNEL_ID_KEY, channel->channelId(), "userId", userId, "renderMode", renderMode, "mirrorMode", mirrorMode);
		...
		{
            LOG_JSON(SET_DEFAULT_MUTE_ALL_REMOTE_AUDIO_STREAMS, CHANNEL_ID_KEY, channel->channelId(), "mute", mute);
		...
		{
            LOG_JSON(SET_DEFAULT_MUTE_ALL_REMOTE_VIDEO_STREAMS, CHANNEL_ID_KEY, channel->channelId(), "mute", mute);
		...
		{
            LOG_JSON(MUTE_ALL_REMOTE_AUDIO_STREAMS, CHANNEL_ID_KEY, channel->channelId(), "mute", mute);
			return channel->muteAllRemoteAudioStreams(mute);
		}

		int
		RtcChannelBridge::adjustUserPlaybackSignalVolume(rtc::uid_t userId, int volume)
		{
            LOG_JSON(ADJUST_USER_PLAYBACK_SIGNAL_VOLUME, CHANNEL_ID_KEY, channel->channelId(), "userId", userId, "volume", volume);
			return channel->adjustUserPlaybackSignalVolume(userId, volume);
		}

		int
		RtcChannelBridge::muteRemoteAudioStream(rtc::uid_t userId, bool mute)
		{
            LOG_JSON(MUTE_REMOTE_AUDIO_STREAM, CHANNEL_ID_KEY, channel->channelId(), "userId", userId, "mute", mute);
		...
		{
            LOG_JSON(MUTE_ALL_REMOTE_VIDEO_STREAMS, CHANNEL_ID_KEY, channel->channelId(), "mute", mute);
			return channel->muteAllRemoteVideoStreams(mute);
		}

		int
		RtcChannelBridge::muteRemoteVideoStream(rtc::uid_t userId, bool mute)
		{
            LOG_JSON(MUTE_REMOTE_VIDEO_STREAM, CHANNEL_ID_KEY, channel->channelId(), "userId", userId, "mute", mute);
			return channel->muteRemoteVideoStream(userId, mute);
		}

		int
		RtcChannelBridge::setRemoteVideoStreamType(rtc::uid_t userId,
			REMOTE_VIDEO_STREAM_TYPE streamType)
		{
            LOG_JSON(SET_REMOTE_VIDEO_STREAM_TYPE, CHANNEL_ID_KEY, channel->channelId(), "userId", userId, "streamType", streamType);
		...
		{
            LOG_JSON(SET_REMOTE_DEFAULT_VIDEO_STREAM_TYPE, CHANNEL_ID_KEY, channel->channelId(), "streamType", streamType);
		...
		{
            LOG_JSON(ADD_PUBLISH_STREAM_URL, CHANNEL_ID_KEY, channel->channelId(), "url", url, "transcodingEnabled", transcodingEnabled);
		...
		{
            LOG_JSON(REMOVE_PUBLISH_STREAM_URL, CHANNEL_ID_KEY, channel->channelId(), "url", url);
		...
		{
            LOG_JSON(SET_LIVE_TRANSCODING, CHANNEL_ID_KEY, channel->channelId(), "transcoding", transcoding);
		...
		{
            LOG_JSON(ADD_INJECT_STREAM_URL, CHANNEL_ID_KEY, channel->channelId(), "url", url, "config", config);
		...
		{
            LOG_JSON(REMOVE_INJECT_STREAM_URL, CHANNEL_ID_KEY, channel->channelId(), "url", url);
		...
		{
            LOG_JSON(START_CHANNEL_MEDIA_RELAY, CHANNEL_ID_KEY, channel->channelId(), "config", configuration);
		...
		{
            LOG_JSON(UPDATE_CHANNEL_MEDIA_RELAY, CHANNEL_ID_KEY, channel->channelId(), "config", configuration);
        ...
        {
            LOG_JSON(SEND_METADATA, CHANNEL_ID_KEY, channel->channelId(), "metadata", *metadata);
            if (metadataObserver) {
        ...
        {
            LOG_JSON(SET_MAX_META_SIZE, CHANNEL_ID_KEY, channel->channelId(), "size", size);
            if (metadataObserver) {
        ...
        rtc::IChannel* RtcChannelBridge::getChannel()
        {
            return channel;
        }
	}
