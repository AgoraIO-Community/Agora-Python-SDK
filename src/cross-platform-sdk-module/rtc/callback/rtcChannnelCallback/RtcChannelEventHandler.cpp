...
namespace common {
    ...
    void RtcChannelEventHandler::initChannelCallbackEvent(CChannelEngineEventHandler* channelEngineEvent)
    {
        cChannelEngineEvent->onWarning = channelEngineEvent->onWarning;
        cChannelEngineEvent->onError = channelEngineEvent->onError;
        cChannelEngineEvent->onJoinChannelSuccess = channelEngineEvent->onJoinChannelSuccess;
        cChannelEngineEvent->onRejoinChannelSuccess = channelEngineEvent->onRejoinChannelSuccess;
        cChannelEngineEvent->onLeaveChannel = channelEngineEvent->onLeaveChannel;
        cChannelEngineEvent->onClientRoleChanged = channelEngineEvent->onClientRoleChanged;
        ...
        cChannelEngineEvent->onTokenPrivilegeWillExpire = channelEngineEvent->onTokenPrivilegeWillExpire;
        ...
        cChannelEngineEvent->onRemoteVideoStats = channelEngineEvent->onRemoteVideoStats;
        cChannelEngineEvent->onRemoteAudioStats = channelEngineEvent->onRemoteAudioStats;
        cChannelEngineEvent->onRemoteAudioStateChanged = channelEngineEvent->onRemoteAudioStateChanged;
        cChannelEngineEvent->onActiveSpeaker = channelEngineEvent->onActiveSpeaker;
        cChannelEngineEvent->onVideoSizeChanged = channelEngineEvent->onVideoSizeChanged;
        cChannelEngineEvent->onRemoteVideoStateChanged = channelEngineEvent->onRemoteVideoStateChanged;
        cChannelEngineEvent->onStreamMessage = channelEngineEvent->onStreamMessage;
        cChannelEngineEvent->onStreamMessageError = channelEngineEvent->onStreamMessageError;
        cChannelEngineEvent->onMediaRelayStateChanged = channelEngineEvent->onMediaRelayStateChanged;
        cChannelEngineEvent->onMediaRelayEvent = channelEngineEvent->onMediaRelayEvent;
        cChannelEngineEvent->onRtmpStreamingStateChanged = channelEngineEvent->onRtmpStreamingStateChanged;
        cChannelEngineEvent->onTranscodingUpdated = channelEngineEvent->onTranscodingUpdated;
        cChannelEngineEvent->onStreamInjectedStatus = channelEngineEvent->onStreamInjectedStatus;
        ...
        rtc::uid_t uid, int elapsed)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onJoinChannelSuccess)
            cChannelEngineEvent->onJoinChannelSuccess(rtcChannel->channelId(), uid, elapsed);
        ...
        int elapsed)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onRejoinChannelSuccess)
            cChannelEngineEvent->onRejoinChannelSuccess(rtcChannel->channelId(), uid, elapsed);
        ...
        CLIENT_ROLE_TYPE newRole)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onClientRoleChanged)
            cChannelEngineEvent->onClientRoleChanged(rtcChannel->channelId(), int(oldRole), int(newRole));
        ...
        const char* token)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onTokenPrivilegeWillExpire)
            cChannelEngineEvent->onTokenPrivilegeWillExpire(rtcChannel->channelId(), token);
        ...
        const RtcStats& stats)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onRtcStats)
            cChannelEngineEvent->onRtcStats(rtcChannel->channelId(), stats.duration, stats.txBytes, stats.rxBytes, stats.txAudioBytes, stats.txVideoBytes, stats.rxAudioBytes, stats.rxVideoBytes, stats.txKBitRate, stats.rxKBitRate, stats.rxAudioKBitRate, stats.txAudioKBitRate, stats.rxVideoKBitRate, stats.txVideoKBitRate, stats.lastmileDelay, stats.txPacketLossRate, stats.rxPacketLossRate, stats.userCount, stats.cpuAppUsage, stats.cpuTotalUsage, stats.gatewayRtt, stats.memoryAppUsageRatio, stats.memoryTotalUsageRatio, stats.memoryAppUsageInKbytes);
        ...
        int rxQuality)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onNetworkQuality)
            cChannelEngineEvent->onNetworkQuality(rtcChannel->channelId(), uid, txQuality, rxQuality);
        ...
        const RemoteVideoStats& stats)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onRemoteVideoStats)
            cChannelEngineEvent->onRemoteVideoStats(rtcChannel->channelId(), stats.uid, stats.delay, stats.width, stats.height, stats.receivedBitrate, stats.decoderOutputFrameRate, stats.rendererOutputFrameRate, stats.packetLossRate, (int)stats.rxStreamType, stats.totalFrozenTime, stats.frozenRate, stats.totalActiveTime);
        ...
        const RemoteAudioStats& stats)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onRemoteAudioStats)
            cChannelEngineEvent->onRemoteAudioStats(rtcChannel->channelId(), stats.uid, stats.quality, stats.networkTransportDelay, stats.jitterBufferDelay, stats.audioLossRate, stats.numChannels, stats.receivedSampleRate, stats.receivedBitrate, stats.totalFrozenTime, stats.frozenRate, stats.totalActiveTime);
        ...
        REMOTE_AUDIO_STATE_REASON reason, int elapsed)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onRemoteAudioStateChanged)
            cChannelEngineEvent->onRemoteAudioStateChanged(rtcChannel->channelId(), uid, int(state), int(reason), elapsed);
        ...
        int height, int rotation)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onVideoSizeChanged)
            cChannelEngineEvent->onVideoSizeChanged(rtcChannel->channelId(), uid, width, height, rotation);
        ...
        REMOTE_VIDEO_STATE_REASON reason, int elapsed)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onRemoteVideoStateChanged)
            cChannelEngineEvent->onRemoteVideoStateChanged(rtcChannel->channelId(), uid, int(state), int(reason), elapsed);
        ...
        const char* data, size_t length)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onStreamMessage)
            cChannelEngineEvent->onStreamMessage(rtcChannel->channelId(), uid, streamId, data, length);
        ...
        int cached)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onStreamMessageError)
            cChannelEngineEvent->onStreamMessageError(rtcChannel->channelId(), uid, streamId, code, missed, cached);
        ...
        CHANNEL_MEDIA_RELAY_ERROR code)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onMediaRelayStateChanged)
            cChannelEngineEvent->onMediaRelayStateChanged(rtcChannel->channelId(), int(state), int(code));
        ...
        RTMP_STREAM_PUBLISH_ERROR errCode)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onRtmpStreamingStateChanged)
            cChannelEngineEvent->onRtmpStreamingStateChanged(rtcChannel->channelId(), url, int(state), int(errCode));
        ...
        int status)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onStreamInjectedStatus)
            cChannelEngineEvent->onStreamInjectedStatus(rtcChannel->channelId(), url, uid, status);
    ...
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onLocalPublishFallbackToAudioOnly)
            cChannelEngineEvent->onLocalPublishFallbackToAudioOnly(rtcChannel->channelId(), isFallbackOrRecover);
    ...
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onRemoteSubscribeFallbackToAudioOnly)
            cChannelEngineEvent->onRemoteSubscribeFallbackToAudioOnly(rtcChannel->channelId(), uid, isFallbackOrRecover);
        ...
        CONNECTION_CHANGED_REASON_TYPE reason)
    {
        if (cChannelEngineEvent && cChannelEngineEvent->onConnectionStateChanged)
            cChannelEngineEvent->onConnectionStateChanged(rtcChannel->channelId(), int(state), int(reason));
