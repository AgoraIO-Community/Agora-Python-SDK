...
cdef class pyIRtmService:
	...
	@property
	def addition(self):
		return self.c_IRtmService.addition
	@addition.setter
	def addition(self, bool addition):
		self.c_IRtmService.addition=addition
	def __cinit__(self):
		self.c_IRtmService = new IRtmService()
	def destruct(self):
		free(self.c_IRtmService)
	def __dealloc__(self):
		pass
...
cdef class pyIAgoraService:
	...
	def createRtmService(self):
		tmp = pyIRtmService()
		tmp.c_IRtmService=(self.c_IAgoraService.createRtmService())
		return tmp
...
class pyLOCAL_VIDEO_STREAM_STATE:
	LOCAL_VIDEO_STREAM_STATE_STOPPED = 0
	LOCAL_VIDEO_STREAM_STATE_CAPTURING = 1
	LOCAL_VIDEO_STREAM_STATE_ENCODING = 2
	LOCAL_VIDEO_STREAM_STATE_FAILED = 3
class pyLOCAL_VIDEO_STREAM_ERROR:
...
class pyRTMP_STREAM_PUBLISH_STATE:
	RTMP_STREAM_PUBLISH_STATE_IDLE = 0
	RTMP_STREAM_PUBLISH_STATE_CONNECTING = 1
	RTMP_STREAM_PUBLISH_STATE_RUNNING = 2
	RTMP_STREAM_PUBLISH_STATE_RECOVERING = 3
	RTMP_STREAM_PUBLISH_STATE_FAILURE = 4
class pyRTMP_STREAM_PUBLISH_ERROR:
...
class pyAUDIO_CODEC_PROFILE_TYPE:
...
cdef class pyRtcStats:
	...
	@property
	def txPacketLossRate(self):
		return self.c_RtcStats.txPacketLossRate
	@txPacketLossRate.setter
	def txPacketLossRate(self, unsigned short txPacketLossRate):
		self.c_RtcStats.txPacketLossRate=txPacketLossRate
	@property
	def rxPacketLossRate(self):
		return self.c_RtcStats.rxPacketLossRate
	@rxPacketLossRate.setter
	def rxPacketLossRate(self, unsigned short rxPacketLossRate):
		self.c_RtcStats.rxPacketLossRate=rxPacketLossRate
	@property
	def userCount(self):
...
cdef class pyLocalVideoStats:
	...
	@property
	def encoderOutputFrameRate(self):
		return self.c_LocalVideoStats.encoderOutputFrameRate
	@encoderOutputFrameRate.setter
	def encoderOutputFrameRate(self, int encoderOutputFrameRate):
		self.c_LocalVideoStats.encoderOutputFrameRate=encoderOutputFrameRate
	@property
	def rendererOutputFrameRate(self):
		return self.c_LocalVideoStats.rendererOutputFrameRate
	@rendererOutputFrameRate.setter
	def rendererOutputFrameRate(self, int rendererOutputFrameRate):
		self.c_LocalVideoStats.rendererOutputFrameRate=rendererOutputFrameRate
	@property
...
cdef class pyRemoteVideoStats:
	...
	@property
	def decoderOutputFrameRate(self):
		return self.c_RemoteVideoStats.decoderOutputFrameRate
	@decoderOutputFrameRate.setter
	def decoderOutputFrameRate(self, int decoderOutputFrameRate):
		self.c_RemoteVideoStats.decoderOutputFrameRate=decoderOutputFrameRate
	@property
	def rendererOutputFrameRate(self):
		return self.c_RemoteVideoStats.rendererOutputFrameRate
	@rendererOutputFrameRate.setter
	def rendererOutputFrameRate(self, int rendererOutputFrameRate):
		self.c_RemoteVideoStats.rendererOutputFrameRate=rendererOutputFrameRate
	@property
...
cdef class pyLiveTranscoding:
	...
	@property
	def audioCodecProfile(self):
		return self.c_LiveTranscoding.audioCodecProfile
	@audioCodecProfile.setter
	def audioCodecProfile(self, AUDIO_CODEC_PROFILE_TYPE audioCodecProfile):
		self.c_LiveTranscoding.audioCodecProfile=audioCodecProfile
	def __cinit__(self):
...
cdef class pyScreenCaptureParameters:
	...
	@property
	def captureMouseCursor(self):
		return self.c_ScreenCaptureParameters.captureMouseCursor
	@captureMouseCursor.setter
	def captureMouseCursor(self, bool captureMouseCursor):
		self.c_ScreenCaptureParameters.captureMouseCursor=captureMouseCursor
	def __cinit__(self):
	...
	def construct_1(self):
		self.c_ScreenCaptureParameters = new ScreenCaptureParameters()
	def construct_2(self,pyVideoDimensions x1,int x2,int x3,bool x4):
		self.c_ScreenCaptureParameters = new ScreenCaptureParameters(cython.operator.dereference(x1.c_VideoDimensions),x2,x3,x4)
	def construct_3(self,int x1,int x2,int x3,int x4,bool x5):
		self.c_ScreenCaptureParameters = new ScreenCaptureParameters(x1,x2,x3,x4,x5)
...
cdef class pyRtcEngineContext:
	...
	def construct_1(self):
		self.c_RtcEngineContext = new RtcEngineContext()
from IAgoraRtcEngine cimport IMetadataObserver
cdef class pyIMetadataObserver:
	cdef IMetadataObserver *c_IMetadataObserver
	def getMaxMetadataSize(self):
		return self.c_IMetadataObserver.getMaxMetadataSize()
...
class pyMETADATA_TYPE:
	UNKNOWN_METADATA = -1
	VIDEO_METADATA = 0
from IAgoraRtcEngine cimport Metadata
cdef class pyMetadata:
	cdef Metadata *c_Metadata
	@property
	def uid(self):
		return self.c_Metadata.uid
	@uid.setter
	def uid(self, unsigned int uid):
		self.c_Metadata.uid=uid
	@property
	def size(self):
		return self.c_Metadata.size
	@size.setter
	def size(self, unsigned int size):
		self.c_Metadata.size=size
	@property
	def buffer(self):
		return cython.operator.dereference(self.c_Metadata.buffer)
	@buffer.setter
	def buffer(self, buffer):
		self.c_Metadata.buffer=buffer
	@property
	def timeStampMs(self):
		return self.c_Metadata.timeStampMs
	@timeStampMs.setter
	def timeStampMs(self, long long timeStampMs):
		self.c_Metadata.timeStampMs=timeStampMs
	def __cinit__(self):
		self.c_Metadata = new Metadata()
	def destruct(self):
		free(self.c_Metadata)
	def __dealloc__(self):
		pass
...
cdef class pyIRtcEngine:
	...
	def setCameraCapturerConfiguration(self,pyCameraCapturerConfiguration x1):
		return self.c_IRtcEngine.setCameraCapturerConfiguration(cython.operator.dereference(x1.c_CameraCapturerConfiguration))
	...
	def muteLocalAudioStream(self,bool x1):
		return self.c_IRtcEngine.muteLocalAudioStream(x1)
	def muteAllRemoteAudioStreams(self,bool x1):
		return self.c_IRtcEngine.muteAllRemoteAudioStreams(x1)
	def setDefaultMuteAllRemoteAudioStreams(self,bool x1):
		return self.c_IRtcEngine.setDefaultMuteAllRemoteAudioStreams(x1)
	def muteRemoteAudioStream(self,unsigned int x1,bool x2):
		return self.c_IRtcEngine.muteRemoteAudioStream(x1,x2)
	def muteLocalVideoStream(self,bool x1):
		return self.c_IRtcEngine.muteLocalVideoStream(x1)
	def enableLocalVideo(self,bool x1):
		return self.c_IRtcEngine.enableLocalVideo(x1)
	def muteAllRemoteVideoStreams(self,bool x1):
		return self.c_IRtcEngine.muteAllRemoteVideoStreams(x1)
	def setDefaultMuteAllRemoteVideoStreams(self,bool x1):
		return self.c_IRtcEngine.setDefaultMuteAllRemoteVideoStreams(x1)
	def muteRemoteVideoStream(self,unsigned int x1,bool x2):
		return self.c_IRtcEngine.muteRemoteVideoStream(x1,x2)
	def setRemoteVideoStreamType(self,unsigned int x1,REMOTE_VIDEO_STREAM_TYPE x2):
		return self.c_IRtcEngine.setRemoteVideoStreamType(x1,x2)
	def setRemoteDefaultVideoStreamType(self,REMOTE_VIDEO_STREAM_TYPE x1):
		return self.c_IRtcEngine.setRemoteDefaultVideoStreamType(x1)
	def enableAudioVolumeIndication(self,int x1,int x2):
		return self.c_IRtcEngine.enableAudioVolumeIndication(x1,x2)
	def startAudioRecording(self,char*x1,AUDIO_RECORDING_QUALITY_TYPE x2):
		return self.c_IRtcEngine.startAudioRecording(x1,x2)
	def stopAudioRecording(self):
		return self.c_IRtcEngine.stopAudioRecording()
	def startAudioMixing(self,char*x1,bool x2,bool x3,int x4):
		return self.c_IRtcEngine.startAudioMixing(x1,x2,x3,x4)
	def stopAudioMixing(self):
		return self.c_IRtcEngine.stopAudioMixing()
	def pauseAudioMixing(self):
		return self.c_IRtcEngine.pauseAudioMixing()
	def resumeAudioMixing(self):
		return self.c_IRtcEngine.resumeAudioMixing()
	def adjustAudioMixingVolume(self,int x1):
		return self.c_IRtcEngine.adjustAudioMixingVolume(x1)
	def adjustAudioMixingPlayoutVolume(self,int x1):
		return self.c_IRtcEngine.adjustAudioMixingPlayoutVolume(x1)
	def getAudioMixingPlayoutVolume(self):
		return self.c_IRtcEngine.getAudioMixingPlayoutVolume()
	def adjustAudioMixingPublishVolume(self,int x1):
		return self.c_IRtcEngine.adjustAudioMixingPublishVolume(x1)
	def getAudioMixingPublishVolume(self):
		return self.c_IRtcEngine.getAudioMixingPublishVolume()
	def getAudioMixingDuration(self):
		return self.c_IRtcEngine.getAudioMixingDuration()
	def getAudioMixingCurrentPosition(self):
		return self.c_IRtcEngine.getAudioMixingCurrentPosition()
	def setAudioMixingPosition(self,int x1):
		return self.c_IRtcEngine.setAudioMixingPosition(x1)
	def getEffectsVolume(self):
		return self.c_IRtcEngine.getEffectsVolume()
	def setEffectsVolume(self,int x1):
		return self.c_IRtcEngine.setEffectsVolume(x1)
	def setVolumeOfEffect(self,int x1,int x2):
		return self.c_IRtcEngine.setVolumeOfEffect(x1,x2)
	def playEffect(self,int x1,char*x2,int x3,double x4,double x5,int x6,bool x7):
		return self.c_IRtcEngine.playEffect(x1,x2,x3,x4,x5,x6,x7)
	def stopEffect(self,int x1):
		return self.c_IRtcEngine.stopEffect(x1)
	def stopAllEffects(self):
		return self.c_IRtcEngine.stopAllEffects()
	def preloadEffect(self,int x1,char*x2):
		return self.c_IRtcEngine.preloadEffect(x1,x2)
	def unloadEffect(self,int x1):
		return self.c_IRtcEngine.unloadEffect(x1)
	def pauseEffect(self,int x1):
		return self.c_IRtcEngine.pauseEffect(x1)
	def pauseAllEffects(self):
		return self.c_IRtcEngine.pauseAllEffects()
	def resumeEffect(self,int x1):
		return self.c_IRtcEngine.resumeEffect(x1)
	def resumeAllEffects(self):
		return self.c_IRtcEngine.resumeAllEffects()
	def enableSoundPositionIndication(self,bool x1):
		return self.c_IRtcEngine.enableSoundPositionIndication(x1)
	def setRemoteVoicePosition(self,int x1,double x2,double x3):
		return self.c_IRtcEngine.setRemoteVoicePosition(x1,x2,x3)
	def setLocalVoicePitch(self,double x1):
		return self.c_IRtcEngine.setLocalVoicePitch(x1)
	def setLocalVoiceEqualization(self,AUDIO_EQUALIZATION_BAND_FREQUENCY x1,int x2):
		return self.c_IRtcEngine.setLocalVoiceEqualization(x1,x2)
	def setLocalVoiceReverb(self,AUDIO_REVERB_TYPE x1,int x2):
		return self.c_IRtcEngine.setLocalVoiceReverb(x1,x2)
	def setLocalVoiceChanger(self,VOICE_CHANGER_PRESET x1):
		return self.c_IRtcEngine.setLocalVoiceChanger(x1)
	def setLocalVoiceReverbPreset(self,AUDIO_REVERB_PRESET x1):
		return self.c_IRtcEngine.setLocalVoiceReverbPreset(x1)
	def setLogFile(self,char*x1):
		return self.c_IRtcEngine.setLogFile(x1)
	def setLogFilter(self,unsigned int x1):
		return self.c_IRtcEngine.setLogFilter(x1)
	def setLogFileSize(self,unsigned int x1):
		return self.c_IRtcEngine.setLogFileSize(x1)
	def setLocalRenderMode(self,RENDER_MODE_TYPE x1):
		return self.c_IRtcEngine.setLocalRenderMode(x1)
	def setRemoteRenderMode(self,unsigned int x1,RENDER_MODE_TYPE x2):
		return self.c_IRtcEngine.setRemoteRenderMode(x1,x2)
	def setLocalVideoMirrorMode(self,VIDEO_MIRROR_MODE_TYPE x1):
		return self.c_IRtcEngine.setLocalVideoMirrorMode(x1)
	def enableDualStreamMode(self,bool x1):
		return self.c_IRtcEngine.enableDualStreamMode(x1)
	def setExternalAudioSource(self,bool x1,int x2,int x3):
		return self.c_IRtcEngine.setExternalAudioSource(x1,x2,x3)
	def setExternalAudioSink(self,bool x1,int x2,int x3):
		return self.c_IRtcEngine.setExternalAudioSink(x1,x2,x3)
	def setRecordingAudioFrameParameters(self,int x1,int x2,RAW_AUDIO_FRAME_OP_MODE_TYPE x3,int x4):
		return self.c_IRtcEngine.setRecordingAudioFrameParameters(x1,x2,x3,x4)
	def setPlaybackAudioFrameParameters(self,int x1,int x2,RAW_AUDIO_FRAME_OP_MODE_TYPE x3,int x4):
		return self.c_IRtcEngine.setPlaybackAudioFrameParameters(x1,x2,x3,x4)
	def setMixedAudioFrameParameters(self,int x1,int x2):
		return self.c_IRtcEngine.setMixedAudioFrameParameters(x1,x2)
	def adjustRecordingSignalVolume(self,int x1):
		return self.c_IRtcEngine.adjustRecordingSignalVolume(x1)
	def adjustPlaybackSignalVolume(self,int x1):
		return self.c_IRtcEngine.adjustPlaybackSignalVolume(x1)
	def enableWebSdkInteroperability(self,bool x1):
		return self.c_IRtcEngine.enableWebSdkInteroperability(x1)
	def setLocalPublishFallbackOption(self,STREAM_FALLBACK_OPTIONS x1):
		return self.c_IRtcEngine.setLocalPublishFallbackOption(x1)
	def setRemoteSubscribeFallbackOption(self,STREAM_FALLBACK_OPTIONS x1):
		return self.c_IRtcEngine.setRemoteSubscribeFallbackOption(x1)
	...
	def registerMediaMetadataObserver(self,pyIMetadataObserver x1,METADATA_TYPE x2):
		return self.c_IRtcEngine.registerMediaMetadataObserver(x1.c_IMetadataObserver,x2)
...
cdef class pyRtcEngineParameters:
	...
	def getAudioMixingPlayoutVolume(self):
		return self.c_RtcEngineParameters.getAudioMixingPlayoutVolume()
	def adjustAudioMixingPublishVolume(self,int x1):
		return self.c_RtcEngineParameters.adjustAudioMixingPublishVolume(x1)
	def getAudioMixingPublishVolume(self):
		return self.c_RtcEngineParameters.getAudioMixingPublishVolume()
	def getAudioMixingDuration(self):
...
cdef class pyAudioFrame:
	...
	@renderTimeMs.setter
	def renderTimeMs(self, long long renderTimeMs):
